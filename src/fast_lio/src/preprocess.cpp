...
Preprocess::Preprocess()
  : time_unit_scale(1.e-3f), lidar_type(AVIA), point_filter_num(1),
    N_SCANS(6), SCAN_RATE(10), time_unit(US), blind(0.01),
    feature_enabled(false), given_offset_time(false)
{
  inf_bound = 10;
  group_size = 8;
  disA = 0.01;
...
bool Preprocess::filtered_out(double x, double y, double z) const
{
  // URDF 还没到时不生效，只靠 blind 兜底。
  if (!self_filter || !self_filter->ready()) {
    return false;
  }
  // 几何体是在 base_link 系下描述的，所以先把点用完整的 T_lidar^base 转过去
  // （旋转 + 平移，两者都不能少）。
  const Eigen::Vector3d p_robot =
    R_lidar_to_robot * Eigen::Vector3d(x, y, z) + t_lidar_to_robot;
  return self_filter->inside(p_robot);
}
      ...
      if((msg->points[i].line < N_SCANS) && ((msg->points[i].tag & 0x30) == 0x10 || (msg->points[i].tag & 0x30) == 0x00))
      {
        ...
        if (valid_num % point_filter_num == 0)
        {
          ...
          if(((abs(pl_full[i].x - pl_full[i-1].x) > 1e-7) 
              ...
              && !filtered_out(pl_full[i].x, pl_full[i].y, pl_full[i].z))
          {
...
void Preprocess::airy_handler(const Pcl2MsgConstPtr& msg) {
  ...
  if (feature_enabled)
  {
    ...
    for (uint i = 0; i < plsize; i++)
    {
      ...
      added_pt.curvature = (pl_orig.points[i].timestamp - time_stamp) * time_unit_scale;
      if(pl_orig.points[i].ring < N_SCANS)
      {
        pl_buff[pl_orig.points[i].ring].push_back(added_pt);
      }
